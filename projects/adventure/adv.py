...
# build backtrack path to closest unknown '?'
def find_unknown(start):
    q = Queue()
    visited = set()
    q.enqueue([start])
    ...
    while q.size() > 0:
        ...
        if v not in visited:
            ...
            for direction, connected_room in room:
                if connected_room == "?":
                    return { 'path': build_traversal(path), 'move': direction }
                else:
                    new_path = list(path)
                    new_path.append(connected_room)
                    q.enqueue(new_path)

# takes list of rooms, builds list of traversal steps between
def build_traversal(moves):
    output = []
    for i in range(0, len(moves) - 1):
        for direction, connected_room in graph[moves[i]].items():
            if connected_room == moves[i + 1]:
    ...
    for direction, room in graph[currentRoom.id].items():
        ...
        if room == '?':
            ...
            if currentRoom.id in graph:
                ...
                graph[currentRoom.id][opposites[direction]] = prevRoom.id
            moved = True
            # print('aftermove', graph)
            break
    # room had no ?, so player still in same room
    # no connected unknown room --> BFS to find closest unknown
    if not moved:
        # find back track path and new direction to take
        back_track = find_unknown(currentRoom.id)
        traversalPath += back_track['path']

        # move player along back track to closest room with '?'
        for move in back_track['path']:
            player.travel(move)
            # print('backtracked:', move)
        # store closest with '?' as prev
        prevRoom = player.currentRoom
        # move player into closest '?', store as current
        player.travel(back_track['move'])
        # print('new?:', back_track['move'])
        currentRoom = player.currentRoom
        # append traversal
        traversalPath.append(back_track['move'])
        # update prev room in graph
        # print('prevRoom.id', prevRoom.id)
        graph[prevRoom.id][back_track['move']] = currentRoom.id
        # make entry for new room
        if currentRoom.id in graph:
                graph[currentRoom.id][opposites[back_track['move']]] = prevRoom.id
        else:
            graph[currentRoom.id] = {}
            for exit in currentRoom.getExits():
                graph[currentRoom.id][exit] = '?'
